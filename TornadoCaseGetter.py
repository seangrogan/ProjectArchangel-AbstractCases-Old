...
class TornadoCaseGetter:
    ...
    @staticmethod
    def __COLLATE_EVENTS(sbws, damage_polygons, waypoints=None):
        sbw_cases = []
        tor_cases = []
    ...
    def plot_polys_with_pts(self, date, out_path=None):
        if out_path is None:
            out_path = "./plots"
        automkdir(out_path)
        _, tornadoes, sbws = self.get_specific_case(date)
        polygons = sbws['geometry']
        waypoints = sbws['waypoints']
        ...
        if len(polygons) > 0:
            for polygon in polygons:
                ...
                x, y = zip(*list(set(itertools.chain(*waypoints.tolist()))))
