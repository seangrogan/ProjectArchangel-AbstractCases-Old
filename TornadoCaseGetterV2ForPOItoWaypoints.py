...
class TornadoCaseGetterV2ForPOItoWaypoints:
    ...
    @staticmethod
    def __FILTER_POINTS(waypoints, sbw_geom, pbar=None):
        if pbar:
            pbar.update()
        minx, miny, maxx, maxy = sbw_geom.geometry.bounds
        minx = minx - (maxx - minx)
        miny = miny - (maxy - miny)
        maxx = maxx + (maxx - minx)
        maxy = maxy + (maxy - miny)
        # sbw_minx, sbw_miny, sbw_maxx, sbw_maxy = sbw_geom.geometry.bounds
        # tor_minx, tor_miny, tor_maxx, tor_maxy = torn_geom.geometry.bounds
        # minx, miny, maxx, maxy = min(sbw_minx, tor_minx), min(sbw_miny, tor_miny)
        waypoints = waypoints[
            (waypoints.x >= minx - 1) & (waypoints.x <= maxx + 1) &
            (waypoints.y >= miny - 1) & (waypoints.y <= maxy + 1)
            ]
        waypoints = list(pt for pt in waypoints.index.tolist())
        if waypoints:
            return waypoints
        return np.NaN
